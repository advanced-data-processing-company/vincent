...
TEST_CASE("global log") {
  SUBCASE("critical log") {
    LOG.critical(0, __FILE__, __LINE__, __FUNCTION__, "hello {} log",
                 "critical");
    CHECK(true);
  }
...
TEST_CASE("module log") {
  auto module = LOG.AddModule("module1", LogLevel::ktrace);

  SUBCASE("critical log") {
    LOG.critical(module, __FILE__, __LINE__, __FUNCTION__, "hello {} log",
                 "critical");
    CHECK(true);
  }
  ...
  SUBCASE("warning log") {
    LOG.warn(module, __FILE__, __LINE__, __FUNCTION__, "hello %s log",
             "warning");
    CHECK(true);
  }
...
TEST_CASE("use macro") {
  auto module ADPC_UNUSED = LOG.AddModule("module2", LogLevel::ktrace);

#define CRITICALLOG(format, ...)                                               \
  LOG.critical(module, __FILE__, __LINE__, __FUNCTION__, format, __VA_ARGS__);

#define ERRLOG(format, ...)                                                    \
  LOG.err(module, __FILE__, __LINE__, __FUNCTION__, format, __VA_ARGS__);
