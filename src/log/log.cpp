...
static const string log_pattern{
    "[%C-%m-%d %H:%M:%S] [%^%l%$] [%n][%@] [thread %t] %v"};
...
Log::Log() {
  ...
  // sinks
  daily_file_sink_ =
      make_shared<sinks::daily_file_sink_mt>("logs/daily", 2, 30, true);
  terminal_sink_ = make_shared<spdlog::sinks::stdout_color_sink_mt>();

  // global log
  logPtr log = make_unique<logger>(
      "global", sinks_init_list{daily_file_sink_, terminal_sink_});
...
size_t Log::AddModule(string &&module_name, const LogLevel level,
                      const bool terminal, const bool daily_file) {
...
void Log::Print() const { logs_[0]->debug(print_to_string_stream().str()); }

string Log::PrintToString() const { return print_to_string_stream().str(); }
