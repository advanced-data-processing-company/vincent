...
class ClusterCurl {
  friend class adpctl::Singleton<ClusterCurl>;
...
private:
  ...
  static bool
      s_bGlobalInitStatus; //判断是否已进行全局初始化，0表示未初始化，1表示已经初始化
...
public:
  ClusterCurl();
  ClusterCurl(const std::string &sIP, const unsigned int nPort,
  ...
  //发送消息发送方式
  int sendMsg(const std::string &sMsg, const int nMethod, const int nFormat,
              std::string &sRec);
  ...
  //从文件中读取内容post出去
  int uploadFileContent(const std::string &sFileName, const int nFormat,
                        std::string &sRec);
  ...
  //对单一curl资源进行释放,可用可不用, 不用则留给类析构释放,
  //使用好处时,使用者可以对异常进行处理
...
private:
  ...
  //回调函数，将文件内容读取到缓存中
  static size_t httpReadFile(void *buffer, size_t size, size_t nmemb,
                             void *file);

  //回调函数，将缓存内容写到文件中
  static size_t httpWriteFile(void *buffer, size_t size, size_t nmemb,
                              void *file);

  //回调函数，将返回缓存数据写出
  static size_t httpDataWriter(void *buffer, size_t size, size_t nmemb,
                               void *content);
...
#define HTTP adpctl::Singleton<adpc::ClusterCurl>::GetInstance()
